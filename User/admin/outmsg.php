...
$limmsg=$con->getlastsql("SELECT * FROM `mensajes` WHERE id_envio='$id' ORDER by id_msg DESC","id_msg");
if(isset($_GET['msgn']))
	$limmsg=$_GET['msgn'];
$limmsg++;
...
include("../../resources/theme/User/".$configuser['theme']);
	echo headopen1("Bandeja de salida");
			...
			<div class="row">
                <div class="col-lg-15">
					...
					<table class="table table-bordered table-hover">
						...
						<?php
							 $id=$_SESSION[getUser()];
                              $RES=mysqli_query($con->get(),"SELECT * FROM mensajes WHERE id_envio='$id' and id_msg<=$limmsg order by id_msg desc limit 10");
							$next=0;
							...
							$next--;
							$back=0+$con->getlastsql("SELECT * FROM mensajes WHERE id_envio='$id' and id_msg>$limmsg order by id_msg DESC","id_msg");
							if($back==0)
								$back=$con->getlastsql("SELECT * FROM `mensajes` WHERE id_envio='$id' ORDER by id_msg DESC","id_msg");
								
					    ?>
						
						<tr>
							<td><a  href="outmsg.php?msgn=<?php echo $back?>" class="fa fa-caret-square-o-left fa-lg" style="color:<?php echo $configuser['icon']?>;"></a></td>
							<td  colspan="3"></td>
							<td><a  href="outmsg.php?msgn=<?php echo $next?>" class="fa fa-caret-square-o-right fa-lg" style="color:<?php echo $configuser['icon']?>;"></a></td>
						</tr>
					</table>
